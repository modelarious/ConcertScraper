...
def create_concerts_dataframe(
    ...
    return (
        pd.DataFrame(rows, columns=["name", "date", "city", "state", "country"])
        .sort_values(by="name")
        .drop_duplicates()
    )
...
def create_artist_info_dataframe(
    ...
    links = pd.DataFrame(rows, columns=["name", "hyperlink"])
    concert_counts = concerts[["name", "date"]].groupby("name").count()
    merged = pd.merge(concert_counts, links, how="right", on="name").rename(
        columns={"date": "unique_concert_count"}
    )
    merged["unique_concert_count"] = (
        merged["unique_concert_count"].fillna(0).astype(int)
    )
    return merged
