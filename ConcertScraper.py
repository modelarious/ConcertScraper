...
def create_concerts_dataframe(
    ...
    for name, concerts in artist_concerts.items():
        for concert in concerts:
            rows.append((name, concert.date, concert.city, concert.state, concert.country))

    return pd.DataFrame(rows, columns=["name", "date", "city", "state", "country"]).sort_values(by='name').drop_duplicates()
